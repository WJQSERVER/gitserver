...
async fn upload_pack_v2(
    ...
    match gitserver_core::protocol_v2::parse_command_request(request)? {
        gitserver_core::protocol_v2::Command::LsRefs(req) => Ok(Response::builder()
            ...
            .header(header::CONTENT_TYPE, "application/x-git-upload-pack-result")
            .header(header::CACHE_CONTROL, "no-cache")
            .body(Body::from(gitserver_core::protocol_v2::ls_refs(
                repo_path, &req,
            )?))
            .unwrap()),
        gitserver_core::protocol_v2::Command::Fetch(mut req) => {
            let shallow_update =
                gitserver_core::protocol_v2::apply_shallow_boundaries(repo_path, &mut req)?;
            let is_shallow_negotiation = req.upload_request.shallow.depth.is_some();

            if !req.upload_request.done && !is_shallow_negotiation {
                let common = gitserver_core::protocol_v2::common_haves(repo_path, &req)?;
                let mut body = gitserver_core::protocol_v2::encode_fetch_acknowledgments(&common);
                body.extend_from_slice(&gitserver_core::protocol_v2::encode_shallow_info(
                    &shallow_update,
                ));
            ...
            {
                gitserver_core::protocol_v2::encode_fetch_ready_and_acknowledgments(
                    &gitserver_core::protocol_v2::common_haves(repo_path, &req)?,
                )
            } else {
                Vec::new()
            };
            ...
            body.extend_from_slice(&gitserver_core::pktline::encode(b"packfile\n"));

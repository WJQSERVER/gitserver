...
async fn info_refs_requires_authentication_when_configured() {
    ...
    let server = TestServer::start_with_auth(
        root.path(),
        gitserver_http::AuthConfig {
            basic: Some(gitserver_http::BasicAuthConfig {
                username: "alice".into(),
...
async fn dynamic_registry_can_register_and_unregister_repo_paths() {
    ...
    let registry = std::sync::Arc::new(gitserver_core::discovery::DynamicRepoRegistry::new());
    let state = gitserver_http::SharedState::with_registry(
        registry.clone(),
    ...
    registry
        .register(gitserver_core::discovery::RepoInfo {
...
async fn receive_pack_is_disabled_by_default_policy() {
    ...
    let store =
        gitserver_core::discovery::RepoStore::discover(root.path().to_path_buf(), 0).unwrap();
    let state = gitserver_http::SharedState::with_store_and_auth_policy(
        store,
        gitserver_http::AuthConfig::default(),
        gitserver_http::ServicePolicy::default(),
    );

    let router = gitserver_http::router(state);
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let (shutdown_tx, shutdown_rx) = tokio::sync::oneshot::channel::<()>();
    let handle = tokio::spawn(async move {
...
async fn dynamic_registry_rejects_parent_relative_paths() {
    let root = TempDir::new().unwrap();
    let repo_path = create_bare_repo_with_commits(root.path(), "dynamic.git", 1);
    let state = gitserver_http::SharedState::with_dynamic_registry(
        gitserver_http::AuthConfig::default(),
        gitserver_http::ServicePolicy {
            upload_pack: true,
            upload_pack_v2: true,
    ...
    let err = state
        .register_repo(gitserver_core::discovery::RepoInfo {
            name: "dynamic.git".into(),
    ...
    assert!(matches!(
        err,
        gitserver_core::error::Error::PathTraversal(_)
    ));
}
